            ...
            <div class="movie-list">
                <table>
                    ...
                    <tr>
                        ...
                        <th class="column colimn-large"  align="center">편집</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-for="movie in paginatedMovieList" :key="movie.movie_title">
                        ...
                        <td :style="{ color: 'black' }">
                            <template v-if="movie.isEditing">
                                <input type="text" v-model="movie.start_date" />
                            </template>
                            <template v-else>
                                <span>{{ movie.start_date }}</span>
                            </template>
                        </td>
                        ...
                        <td :style="{ color: 'black' }">
                            <template v-if="movie.isEditing">
                                <input type="text" v-model="movie.end_date" />
                            </template>
                            <template v-else>
                        ...
                        <td :style="{ color: 'black' }">
                            <template v-if="movie.isEditing">
                                <input type="text" v-model="movie.movie_state" />
                            </template>
                            <template v-else>
                                <span>{{ movie.movie_state }}</span>
                            </template>

                        </td>
                        <td style="color:black;width:10%;"  align="center">
                            <template v-if="movie.isEditing">
                                <button @click="toggleEdit(movie)">저장</button>
                            </template>
                            <template v-else>
                                <button @click="toggleEdit(movie)">편집</button>
                            </template>
                        </td>
                    </tr>
                    </tbody>
...
function selectMovies(movie) {
    ...
    if (index > -1) {
        ...
        selectedMovie.isEditing = true; // isEditing 속성을 추가하고 기본값으로 false 설정
...
const editMovie = async(movie) => {
    try {
        const { movie_id,movie_title, movie_state, start_date, end_date } = movie;

        // 서버로 수정된 영화 정보 전송
        await axios.post(`http://localhost:9212/api/getUpdateMovie`, {
            movie_id,
            movie_title,
            movie_state,
            start_date,
